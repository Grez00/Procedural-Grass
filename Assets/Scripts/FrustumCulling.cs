...
public class Plane
{
    ...
    public Plane(Vector3 pPoint, Vector3 pNormal)
    {
        p = pPoint;
        normal = pNormal;
    }
...
public class Frustum
{
    ...
    public Frustum(Camera cam)
    {
        ...
        planes[2] = new Plane(cam.transform.position, leftNormal);
        planes[3] = new Plane(cam.transform.position, rightNormal);
        planes[4] = new Plane(nearCenter, nearNormal);
    ...
    public bool PointTest(Vector3 p)
    {
        ...
        {
            if (plane.SignedDistance(p) > 0.0f) return true;
        }
        return false;
    }
    ...
    public bool AABBTest(AABB a)
    {
        foreach (Plane plane in planes)
        {
            float r = a.extents.x * Mathf.Abs(plane.normal.x);
            r += a.extents.y * Mathf.Abs(plane.normal.y);
            r += a.extents.z * Mathf.Abs(plane.normal.z);

            if (-r > plane.SignedDistance(a.center)) return false;
        }
        return true;
    }
}
